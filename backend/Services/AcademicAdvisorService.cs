...
namespace backend.Services
{
    public class AcademicAdvisorService
    {
        ...
        public async Task<string> ProcessAdvisorChatAsync(int lecturerId, int classId, string message)
        {
            try
            {
                // Bước 1: Quét từ khóa để quyết định gọi Tool (Giả lập Function Calling cho tốc độ siêu nhanh)
                string msgLower = message.ToLower();
                string rawDataFromDb = "";
                string actionTaken = "GeneralChat";

                if (msgLower.Contains("lỗi") || msgLower.Contains("sai") || msgLower.Contains("yếu") || msgLower.Contains("phân tích"))
                {
                ...
                else if (msgLower.Contains("tạo bài") || msgLower.Contains("giao bài") || msgLower.Contains("vá lỗi"))
                {
                    ...
                    // Tìm bài học gần nhất của giảng viên này để nhét bài tập vào
                ...
                {
                    // Trò chuyện bình thường về nghiệp vụ sư phạm
                ...
                // Bước 2: Gửi Dữ liệu thô từ C# lên cho Gemini để nó "Gia công" thành lời văn tự nhiên
        ...
        {
            var studentIds = await _context.ClassStudents.Where(cs => cs.ClassId == classId).Select(cs => cs.StudentId).ToListAsync();
            if (!studentIds.Any()) return "Lớp học này chưa có sinh viên nào.";

            var studentEmails = await _context.Users.Where(u => studentIds.Contains(u.Id)).Select(u => u.Email).ToListAsync();
            var recentFails = await _context.Submissions.Where(s => studentEmails.Contains(s.UserEmail) && s.Status != "Accepted").ToListAsync();
        ...
        private async Task<string> Tool_IdentifyAtRiskStudents(int classId)
        {
            ...
            var riskStudents = await _context.Submissions
                .Where(s => studentEmails.Contains(s.UserEmail) && s.Status != "Accepted")
